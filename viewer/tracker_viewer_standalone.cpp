...
class TrackerViewer : private mlabs::balai::framework::BaseApp,
                      ...
                      private vive::ITrackerSubscriber {
  struct Tracker {
  ...
  // tracker update happens in server thread, needs to pretect trackers.
  std::mutex mutex_;
...
//void TouchBegan_(int id, int taps, int x, int y) override {}
  ...
  void DispatchCommand_(vive::Command& cmd) override {
    if (0==memcmp(cmd.cmd, "clear", 6)) {
      ...
      if (ITrackerSubscriber::SendCommand(cmd.cmd, cmd.len)) {
        cmd.Result("sent", 1);
      } else {
        cmd.Result("failed", -1);
      }
...
public:
  TrackerViewer():BaseApp(),
    ...
    BaseApp::window_title_ = "VIVE Tracker Viewer (standalone)";
    BaseApp::width_  = 1280;
    BaseApp::height_ = 720;
  ...
  //
  // vive::ITrackerSubscriber implementation...
  // server start/stop
