...
//  BL_ASSERT(0!=name);
  ...
  void SetFormatFlags_(TEXTURE_FORMAT fmt) { flags_ = uint32((flags_&~FORMAT_MASK)|((fmt<<16)&FORMAT_MASK)); }
  void SetFilterFlags_(TEXTURE_FILTER filter) {
    flags_ = uint32((flags_&~FILTER_MASK)|((filter<<12)&FILTER_MASK));
  }
  void SetAddressFlags_(TEXTURE_ADDRESS s, TEXTURE_ADDRESS t, TEXTURE_ADDRESS r) {
    flags_ = uint32((flags_&~ADDRESS_MASK) | ((0x00000003&s)<<4) | ((0x00000003&t)<<2) | (0x00000003&r));
  }
...
public:
  ...
  virtual bool UpdateImage(int width, int height, TEXTURE_FORMAT fmt, void const* image, bool complete=false)=0;

  // factory function
  static Texture2D* New(uint32 name, bool dynamic=false);

  // create a RGBA sphere texture, can be used for point sprite
  static Texture2D* CreateSphereTexture(uint32 name=0, int sprite_size=64);
};
...
protected:
  int width_, height_, depth_; // size_x, size_y, size_z
...
public:
  ...
  // supporting formats:
  //   unsigned byte : FORMAT_I8, FORMAT_IA8, FORMAT_RGB8, FORMAT_RGBA8
  virtual bool UpdateImage(int width, int height, int depth, TEXTURE_FORMAT fmt, void const* image)=0;
...
public:
  ...
  char const* FindName(Texture const* tex) {
    return tex ? stringPool_.Find(tex->Name()):nullptr;
  }
#endif
  ITexture* FindTexture(char const* name) const {
    return name ? Find(HashName(name)):nullptr;
  }
...
#define GET_TEXTURE_NAME(tex)   nullptr
#endif
