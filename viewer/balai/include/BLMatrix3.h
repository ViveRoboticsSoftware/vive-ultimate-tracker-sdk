...
public:
  ...
  static Matrix3 RotationMatrix(float x, float y, float z) {
    return Matrix3().SetEulerAngles(x, y, z);
  }
  ...
  Vector3 XAxis() const { return Vector3(_11, _21, _31); }

  // local y-axis(0,1,0) in transformed space, i.e. "Down"
  Vector3 YAxis() const { return Vector3(_12, _22, _32); }

  // local z-axis(0,0,1) in transformed space, i.e. "Front"
  Vector3 ZAxis() const { return Vector3(_13, _23, _33); }

  // Get origin(position)
  Vector3 Origin() const { return Vector3(_14, _24, _34); }

  // euler angle estimation in radian
  Vector3 EulerAngles() const {
    return Vector3(atan2f(_32,_33), atan2f(-_31, sqrtf(_32*_32+_33*_33)), atan2f(_21,_11));
  }

  // 
  // move
  void Move(float x, float y, float z, bool localSpace=true) {
    if (localSpace) {
      _14 += x*_11 + y*_12 + z*_13;
      _24 += x*_21 + y*_22 + z*_23;
      _34 += x*_31 + y*_32 + z*_33;
    } else {
      _14 += x;
      _24 += y;
      _34 += z;
...
*/
  ...
  Matrix3 const& SetEulerAngles(float x, float y, float z);

  // axis angles to Matrix3(origin not touch)
  Matrix3 const& SetRotateAxisAngle(Vector3 const& axis, float angle);

  template<typename Quat>
  Matrix3 const& RotateFromQuaternion(Quat const& q) {
    return RotateFromQuaternion(q.w, q.x, q.y, q.z);
  }

  Matrix3 const& RotateFromQuaternion(double w, double x, double y, double z) {
    double xx = x*x + y*y + z*z;
    double rr = w*w + xx;
    if (rr<0.999999 || rr>1.000001) {
      if (xx<=1.0) {
        xx = sqrt(1.0-xx);
        w = (w>=0.0) ? xx:-xx;
      } else {
    ...
    double const _2x  = 2.0*x;
    double const _2y  = 2.0*y;
    double const _2z  = 2.0*z;
    double const _2wx = _2x*w;
    double const _2wy = _2y*w;
    double const _2wz = _2z*w;
    double const _2xx = _2x*x;
    double const _2xy = _2y*x;
    double const _2xz = _2z*x;
    double const _2yy = _2y*y;
    double const _2yz = _2z*y;
    double const _2zz = _2z*z;

    _11 = (float) (1.0 - _2yy - _2zz);
    _12 = (float) (_2xy - _2wz);
    _13 = (float) (_2xz + _2wy);

    _21 = (float) (_2xy + _2wz);
    _22 = (float) (1.0 - _2xx - _2zz);
    _23 = (float) (_2yz - _2wx);

    _31 = (float) (_2xz - _2wy);
    _32 = (float) (_2yz + _2wx);
    _33 = (float) (1.0 - _2xx - _2yy);
  ...
  Matrix3 GetInverse(float* pDeterminant=0) const;
  ...
  V3 PointTransform(V3 const& p) const {
    return V3{_11*p.x + _12*p.y + _13*p.z + _14,
              _21*p.x + _22*p.y + _23*p.z + _24,
              _31*p.x + _32*p.y + _33*p.z + _34};
  }
  Vector3 const& VectorTransform(Vector3& out, Vector3 const& v) const;
  template<typename V3>
  V3 VectorTransform(V3 const& v) const {
    return V3{_11*v.x + _12*v.y + _13*v.z,
...
//-----------------------------------------------------------------------------
inline Matrix3 Matrix3Multiply(Matrix3 const& Parent, Matrix3 const& Child) {
  return Matrix3(
...
//-----------------------------------------------------------------------------
inline Matrix3 operator*(Matrix3 const& Parent, Matrix3 const& Child) {
  return Matrix3Multiply(Parent, Child);
}

// build Covariance Matrix
Matrix3 BuildCovarianceMatrix(Vector3 const* vertices, int cnt, Vector3 const* pMean=0);
