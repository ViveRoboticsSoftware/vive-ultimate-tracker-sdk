...
enum SURFACE_FORMAT {
  ...
  SURFACE_FORMAT_R32F,    // single channel F32
  SURFACE_FORMAT_R16F,    // single channel F16
  // more to come...
...
public:
  ...
  uint32 GetSurfaceSize(uint32& w, uint32& h) const {
    w = width_;
    h = height_;
...
public:
  SurfaceGenerator():
    ...
    width_(0),height_(0),numRenderTargets_(0),multisampleSamples_(0),depthStencilUsage_(0),extraInfo_(0) {
