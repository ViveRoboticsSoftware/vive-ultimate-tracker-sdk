...
public:
  ...
  uint32 AddVertex2D(V2 const& v) {
    return AddVertex2D(v.x, v.y);
  }
  template<typename V2>
  uint32 AddVertex2D(V2 const& v, float s, float t) {
    return AddVertex2D(v.x, v.y, s, t);
  }

  // 3d vertex, x->right, y->down, z->forward
  ...
  uint32 AddVertex(V3 const& v) {
    return AddVertex(v.x, v.y, v.z);
  }
  template<typename V3>
  uint32 AddVertex(V3 const& v, float s, float t) {
    s_=s; t_=t;
    return AddVertex(v.x, v.y, v.z);
  }
  ...
  template<typename V3>
  uint32 AddPointSprite(V3 const& v, float size) {
    return AddPointSprite(v.x, v.y, v.z, size);
  }
  template<typename V3>
  uint32 AddPointSprite(V3 const& v, Color const& color, float size) {
    color_ = color;
    return AddPointSprite(v.x, v.y, v.z, size);
  }
