...
//
  ...
  //  return 0 if failed
  //  return 1 if both front and back face fill mode setting is shared
  //  return 2 if both set front and back face can be set
  virtual int SetPolygonMode_(GFXPOLYGON /*front*/, GFXPOLYGON /*back*/) { return 0; }
...
//  if (world.Equals(worldMatrixStack_[worldStackTop_])) return;
  ...
  void SetViewFromLTM(math::Matrix3 const& ltm) {
    gfxBuildViewMatrixFromLTM(viewMatrixStack_[viewStackTop_], ltm);
    WorldViewProjMatrixDirtyFlags_ |= 2;
  }
  ...
  void SetPerspectiveProjection(float fovY, float near_clip=0.1f, float far_clip=100.0f) {
    gfxBuildPerspectiveProjMatrix(projMatrixStack_[projMatrixTop_], fovY, (float)screenWidth_/(float)screenHeight_, near_clip, far_clip);
    WorldViewProjMatrixDirtyFlags_ |= 4;
  }
  void SetOrthographicProjection(float left, float right, float bottom, float top, float near_clip=0.1f, float far_clip=100.0f) {
    gfxBuildParallelProjMatrix(projMatrixStack_[projMatrixTop_], left, right, bottom, top, near_clip, far_clip);
    WorldViewProjMatrixDirtyFlags_ |= 4;
  }
  ...
  float  GetScreenAspectRatio() const { return (float)screenWidth_/(float)screenHeight_; }
  uint32 GetFramebufferWidth() const { return surfaceWidth_; }
  uint32 GetFramebufferHeight() const { return surfaceHeight_; }
  float  GetFramebufferAspectRatio() const { return (float)surfaceWidth_/(float)surfaceHeight_; }
