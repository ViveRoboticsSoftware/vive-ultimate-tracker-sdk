...
public:
  ...
  bool SeekSet(long offset) {
    ...
    if (offset<=0) {
      ...
      bufPos_ = (uint32) (offset - filePos_);
      return true;
    }

    bufPos_ = bufRead_ = 0;
    filePos_ = (uint32) offset;
  ...
  bool SeekEnd(long offset) {
    ...
    uint32 rel = fileSize_ - (uint32) offset;
...
public:
  ...
  uint32 GetSize() const {
    ...
    return (FOPEN_READ==openMode_) ? fileSize_:BL_BAD_UINT32_VALUE;
  }
