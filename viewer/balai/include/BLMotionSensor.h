...
public:
  SensorFusion():mutex_(),fusion_(),orientation_(UI_ORIENTATION_PORTRAIT),
    gyro_cutoff_(2.0f*math::constants::float_deg_to_rad),gyro_scale_(1.0f),
    fusion_Kp_(0.5f),stable_time_(0.0f),gyro_put_(0),acc_put_(0),compass_put_(0),fusionEnabled_(0) {
    for (int i=0; i<4; ++i) {
      gyroBuffer_[i].Zero();
      accBuffer_[i].Zero();
    }
