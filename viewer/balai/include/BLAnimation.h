...
public:
  ...
  void EvalJointXforms(float animTime, math::Transform* xforms, uint16* indices) const {
    ...
    if (numSampleJoints_) {
      ...
      BL_ASSERT(frame<=(float)numSamples_);
      float const iframe = floorf(frame);
      float const alpha = frame - iframe;
      math::Transform const* keyframe1 = jointKeyArray_ + 2*numInterpJoints_ + ((int)iframe)*numSampleJoints_;
