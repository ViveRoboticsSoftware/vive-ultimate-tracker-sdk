...
protected:
  Application():appTime_(0.0),frameTime_(0.0f),fps_(0.0f),timeScale_(1.0f),pause_(0) {}
  virtual ~Application() {}
...
public:
  ...
  virtual void OnQuit() {}  // last function to be called even if Create() failed.
  ...
  bool Pause(bool pause) {
    bool const last_state = (pause_!=0);
    if (pause!=last_state) {
      pause_ = pause;
      if (!pause) {
