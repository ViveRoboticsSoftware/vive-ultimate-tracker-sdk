...
inline float FastInvSqrt(float s) {
  ...
  u.i = (int)(0x5f375a86L - (u.i>>1)); // initial guess
//  u.i = 0x5f3759dfL - (u.i>>1); // original : John Carmack or Gary Tarolli? Greg Walsh 80s?
...
inline float UnitRandom(uint32 uiSeed = 0xFFFFFFFF) {
  ...
  return constants::float_inv_rand_max*(float)std::rand();
}
...
inline float IntervalRandom(float fMin, float fMax, uint32 uiSeed = 0xFFFFFFFF) {
  ...
  return fMin + (fMax-fMin)*(constants::float_inv_rand_max*(float)std::rand());
}
...
struct Complex {
  ...
  float Norm() const { return sqrtf(Real*Real + Imaginary*Imaginary); }
  float Phase() const { return atan2f(Imaginary, Real); }
