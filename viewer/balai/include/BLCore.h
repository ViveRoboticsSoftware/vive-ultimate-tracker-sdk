 ...
 *          2012/08/09 add BL_HTC_NEW_HOTFIX for compiling on Android
 *                     mm build because htc has customized <new>
 *          2023/03/09 Linux platform
 *
 */
...
// inf/sup
template<typename T> T BL_SUP(T a, T b) { return (a>b) ? a:b; }
template<typename T> T BL_INF(T a, T b) { return (a<b) ? a:b; }
template<typename T, typename... Args> T BL_SUP(T first, Args&&... args) {
  T&& rest = BL_SUP(args...);
  return (first>rest) ? first:rest;
}
template<typename T, typename... Args> T BL_INF(T first, Args&&... args) {
  T&& rest = BL_INF(args...);
  return (first<rest) ? first:rest;
}
...
#ifndef BL_OS_WINDOWS
#define BL_OS_WINDOWS
#endif
...
#define BL_NO_MEMORY_TRACKER
#ifndef BL_NO_NEW_DELETE_OVERLOAD
#define BL_NO_NEW_DELETE_OVERLOAD
#endif
...
// #elif defined(BL_OS_APPLE_iOS) ???
 ...
 #define BL_USE_EXCEPTIONS
 #define BL_EXCEPTION_SPEC_BADALLOC /* nothing */
 #define BL_EXCEPTION_SPEC_NOTHROW  throw()
 ...
 #define BL_ERR(...) printf(__VA_ARGS__)

 // to link librealsense2.a results tons of memory leaks...
 #define BL_NO_MEMORY_TRACKER
 #ifndef BL_NO_NEW_DELETE_OVERLOAD
  #define BL_NO_NEW_DELETE_OVERLOAD
 #endif
...
///////////////////////////////////////////////////////////////////////////////
  ...
  #define BL_COMPILE_ASSERT(expr, msg) static_assert(expr, #msg)
#else
